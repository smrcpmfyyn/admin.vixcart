...
import com.vixcart.admin.intfc.validation.GetAllPremiumPaymentsValiadtor;
 ...
 * @author rifaie
 */
public class GetAllPremiumPaymentsConstraints implements GetAllPremiumPaymentsValiadtor {

    private final GetAllPremiumPayments req;
    private final DBConnect dbc;
    private final MongoConnect mdbc;

    public GetAllPremiumPaymentsConstraints(GetAllPremiumPayments req) throws Exception {
        this.req = req;
    ...
    public String validateOffset() throws Exception {
        ...
        if (validate(req.getPageNo(), regx)) {
            if (validate(req.getMaxEntries(), regx)) {
                ...
                int maxPageNo = dbc.getMaxPageNo("affiliate_premium_payments ", Integer.parseInt(req.getMaxEntries()));
                if (pageNo <= maxPageNo) {
