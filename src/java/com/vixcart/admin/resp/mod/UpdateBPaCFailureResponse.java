 ...
 * @author Vineeth K
 */
public class UpdateBPaCFailureResponse {

    private final UpdateBPaCResult req;
    private final String error;

    public UpdateBPaCFailureResponse(UpdateBPaCResult req, String error) {
        this.req = req;
        this.error = error;
    }
        ...
        for (int i = 1; i < errors.length; i++) {
            ...
            switch (parameter) {
                ...
                case "onlinevisibility":
                    type = req.getOnlinevisibility();
                    resp = type.substring(type.lastIndexOf(" ") + 1);
                    json += "\"" + parameter + "\"" + ":" + "\"" + resp + "\" ,";
                    break;
                case "offlinevisibility":
                    type = req.getOfflinevisibility();
                    resp = type.substring(type.lastIndexOf(" ") + 1);
                    json += "\"" + parameter + "\"" + ":" + "\"" + resp + "\" ,";
                    break;
            }
