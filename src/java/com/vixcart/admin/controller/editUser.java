...
import com.vixcart.admin.processreq.ProcessEditUser;
import com.vixcart.admin.req.mod.EditUser;
import com.vixcart.admin.resp.mod.EditUserFailureResponse;
...
public class editUser extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            EditUser req = new EditUser(at, id, name, designation, address1, address2, place, pin, utype);
            EditUserValidation reqV = new EditUserValidation(req);
            reqV.validation();
            EditUserResult reqR = JSONParser.parseJSONEU(reqV.toString());
            String validSubmission = reqR.getValidationResult();
            UserActivities ua = new UserActivities(req.getAdmin_id(), req.getType(), "edit_user", "management", "valid");
            if (validSubmission.startsWith(CorrectMsg.CORRECT_MESSAGE)) {
                ProcessEditUser process = new ProcessEditUser(req);
                EditUserSuccessResponse SResp = process.processRequest();
                ...
                EditUserFailureResponse FResp = new EditUserFailureResponse(reqR, validSubmission);
