...
import com.vixcart.admin.intfc.validation.GetAllProductTypesValidator;
 ...
 * @author Vineeth K
 */
public class GetAllProductTypesConstraints implements GetAllProductTypesValidator {

    private final GetAllProductTypes req;
    private final DBConnect dbc;
    private final MongoConnect mdbc;

    public GetAllProductTypesConstraints(GetAllProductTypes req) throws Exception {
        this.req = req;
    ...
    public String validateOffset() throws Exception {
        ...
        if (validate(offset, regX)) {
            if (dbc.checkPTypeOffset(req.getNo(), offset) == 0) {
                valid = CorrectMsg.CORRECT_PTYPE_OFFSET;
            } else {
                valid = ErrMsg.ERR_PTYPE_OFFSET_NOT_EXISTS;
            }
        }
        return valid;
    }
