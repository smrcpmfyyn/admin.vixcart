 ...
 * @author rifaie
 */
public class AddPremiumPaymentFailureResponse {
    private final AddPremiumPaymentResult reqR;
    private final String error;

    public AddPremiumPaymentFailureResponse(AddPremiumPaymentResult reqR, String error) {
        this.reqR = reqR;
        this.error = error;
    }
        ...
        for (int i = 1; i < errors.length; i++) {
            ...
            switch (parameter) {
                ...
                case "company":
                    ...
                    break;
                case "ref":
                    String ref = reqR.getRef();
                    resp = ref.substring(ref.lastIndexOf(" ") + 1);
                    json += "\"" + parameter + "\"" + ":" + "\"" + resp + "\" ,";
                    break;
                case "amount":
                    String amount = reqR.getAmount();
                    resp = amount.substring(amount.lastIndexOf(" ") + 1);
                    json += "\"" + parameter + "\"" + ":" + "\"" + resp + "\" ,";
                    break;
            }
        }
